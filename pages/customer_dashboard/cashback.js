...
import CheckInComp from "../../components/dashboard/Checkin";
...
const CashBack = () => {
  ...
  useEffect(() => {
    ...
    //   try {
    //     const res = await axios(profileConfig);
    //   } catch (e) {
    //   }
    // };
    // winnings();
  ...
  useEffect(() => {
    ...
    const fetchBalance = async () => {
      ...
      try {
        ...
        if (!response?.data.payload.content[0]) {
          setwalletBalance(0);
        }
        if (
          response?.data.payload.content[0] &&
          response?.data.payload.content[0].amount
        ) {
          setwalletBalance(response?.data.payload.content[0].amount);
        }
        ...
        //   isClosable: true,
        //   duration: "5000",
        //   title: "Please check your connection and try again2",
        //   position: "top",
        // });
      } finally {
        setIsLoading(false);
